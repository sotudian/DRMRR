...
import pickle
from Robustness_Functions import Evaluation_LamdaMART_MAP_NDCG,Evaluation_LamdaMART_XE,Create_NN_BlackBox_Model,Generate_adversary_NN_Samples
...
FOLDS = [1,2,3,4,5]  
# NN parameters
params_Subs_Model = {'batch_size': 500,
          'epochs': 10,
...
for EPSILON_FGSM in MUs:        
    ...
    for fold in FOLDS:
        ...
        Best_Theta_DRO_MRR = np.load(Add_Theta+'/MED_DRMRR/Best_Theta_MRR_LOSS_L_InfNone_Fold_'+str(fold)+'.npy')   
        ...
        del TX, Ty,Tc,f1,f2,f3,trainfile,Add_Theta,Tqids
         
        # Load data test datsets =========----------------------------------
        with open(Dir_Address+"/test.txt") as testfile:
            TestX, Testy, Testqids, Testc = Read_LTR_Dataset(testfile, has_targets=True , one_indexed=True)
        Test_Unique_QID = np.unique(Testqids).astype(np.float64)    
        del TestX, Testy, Testqids, Testc, testfile,Dir_Address   
        
        #  Perturbed Test Data   
        random.seed(1111)
        Noisy_Test_Unique_QID = list(random.choices(Test_Unique_QID,k=round(len(Test_Unique_QID)*Percent_Noisy)))     
             
        # DRO-MRR 
        Perturbed_Final_X_DRMRR = copy.deepcopy(Final_X) #  Perturbed Test Data   =========--------------------
        Substitute_Reg_DRMRR_Theta = Create_NN_BlackBox_Model(Train_Unique_QID,Best_Theta_DRO_MRR,Final_X,Final_Relevance,'MRR_or_DRMRR',params_Subs_Model)
        for i in range(len(Final_X)):
            if Final_X[i,0] in Noisy_Test_Unique_QID:
                Perturbed_Final_X_DRMRR[i,1:] = Generate_adversary_NN_Samples(Substitute_Reg_DRMRR_Theta, Final_X[i,1:], Final_Y[i,:], EPSILON_FGSM )
              
        Perturbed_DR_MRR_Result = Evaluation_DRMRR(Test_Unique_QID,Best_Theta_DRO_MRR,Perturbed_Final_X_DRMRR,Final_Relevance)
        ...
        # LamdaMart_MAP
        Perturbed_Final_X_Mart_MAP = copy.deepcopy(Final_X) #  Perturbed Test Data   =========--------------------
        Substitute_Reg_Mart_MAP_Theta = Create_NN_BlackBox_Model(Train_Unique_QID,Best_MODEL_LamdaMart_MAP,Final_X,Final_Relevance,'LamdaMART_MAP_NDCG',params_Subs_Model)
        for i in range(len(Final_X)):
            if Final_X[i,0] in Noisy_Test_Unique_QID:
                Perturbed_Final_X_Mart_MAP[i,1:] = Generate_adversary_NN_Samples(Substitute_Reg_Mart_MAP_Theta, Final_X[i,1:], Final_Relevance[i,:], EPSILON_FGSM)
                                     
        Perturbed_LamdaMart_MAP_Result = Evaluation_LamdaMART_MAP_NDCG(Test_Unique_QID,Best_MODEL_LamdaMart_MAP,   np.hstack( [Final_Relevance, Perturbed_Final_X_Mart_MAP ])   )
        Perturbed_All_Result.loc['LamdaMart_MAP'] = list(Perturbed_LamdaMart_MAP_Result)

        # LamdaMart_NDCG
        Perturbed_Final_X_Mart_NDCG = copy.deepcopy(Final_X) #  Perturbed Test Data   =========--------------------
        Substitute_Reg_Mart_NDCG_Theta = Create_NN_BlackBox_Model(Train_Unique_QID,Best_MODEL_LamdaMart_NDCG,Final_X,Final_Relevance,'LamdaMART_MAP_NDCG',params_Subs_Model)
        for i in range(len(Final_X)):
            if Final_X[i,0] in Noisy_Test_Unique_QID:
                Perturbed_Final_X_Mart_NDCG[i,1:] = Generate_adversary_NN_Samples(Substitute_Reg_Mart_NDCG_Theta, Final_X[i,1:], Final_Relevance[i,:], EPSILON_FGSM)
                                          
        Perturbed_LamdaMart_NDCG_Result = Evaluation_LamdaMART_MAP_NDCG(Test_Unique_QID,Best_MODEL_LamdaMart_NDCG,   np.hstack( [Final_Relevance, Perturbed_Final_X_Mart_NDCG ])   )
        Perturbed_All_Result.loc['LamdaMart_NDCG'] = list(Perturbed_LamdaMart_NDCG_Result)

         # XE_NDCG_MART
        Perturbed_Final_X_XE = copy.deepcopy(Final_X) #  Perturbed Test Data   =========--------------------
        Substitute_Reg_XE_Theta = Create_NN_BlackBox_Model(Train_Unique_QID,Best_MODEL_XE_NDCG_MART,Final_X,Final_Relevance,'XE_NDCG_MART',params_Subs_Model)
        for i in range(len(Final_X)):
            if Final_X[i,0] in Noisy_Test_Unique_QID:
                Perturbed_Final_X_XE[i,1:] = Generate_adversary_NN_Samples(Substitute_Reg_XE_Theta, Final_X[i,1:], Final_Relevance[i,:], EPSILON_FGSM)  
                                      
        Perturbed_XE_NDCG_MART_Result = Evaluation_LamdaMART_XE(Test_Unique_QID,Best_MODEL_XE_NDCG_MART,    np.hstack( [Final_Relevance, Perturbed_Final_X_XE ])   )
        ...
        del Perturbed_All_Result,
    
    A = ['Fold'+str(EPSILON_FGSM)]
    ...
    del A,Fold_Row,Folds_Average_Perturbed_All,EPSILON_FGSM
    
del Test_Unique_QID,counter,Count_Ep  
...
with plt.style.context(Graph_Style):
    for N in [5,10]: 
        ...
        plt.title('Black-Box Attack', fontweight="bold")
        plt.xlabel('FGSM Epsilon',size=10)
