...
from Robustness_Functions import Evaluation_LamdaMART_MAP_NDCG,Evaluation_LamdaMART_XE,FGSM_Func_SimpReg
...
for EPSILON_FGSM in MUs:        
    ...
    for fold in FOLDS:
        ...
        Best_Theta_SimpReg = np.load(Add_Theta+'/MED_SimpleReg/Best_Theta_SimpReg_Fold_'+str(fold)+'.npy') 
        ...
        for i in range(len(Final_X)):
            if Final_X[i,0] in Noisy_Test_Unique_QID:
                Perturbed_Final_X_SimpReg[i,1:] = Final_X[i,1:] + FGSM_Func_SimpReg(Best_Theta_SimpReg,Final_X[i,1:],Final_Relevance[i,:],EPSILON_FGSM)
...
with plt.style.context(Graph_Style):
    for N in [5,10]: 
        ...
        plt.title('Universal Perturbation Attack', fontweight="bold")
